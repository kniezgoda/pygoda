...
class DiffZonalMean:
	def __init__(self, parent, control_file = '', test_file = '', box = [-90,90,0,360], var = "PRECT"):
		self.begin = parent
		self.begin.title("DiffZonalMean.py, choose input data...")
...
class DiffMap:
	def __init__(self, parent, control_file = '', test_file = '', box = [-40,40,335,333], var = "PRECT", clev = '', diffclev = ''):
		self.begin = parent
		self.begin.title("DiffMap.py, choose input data...")
		# Things that need defaults
		self.control_filepath = control_file
		self.control_filename = ''
		if self.control_filepath is not '':
...
class DiffMeridionalMean:
	def __init__(self, parent, control_file = '', test_file = '', box = [-90,90,0,360], var = "PRECT"):
		self.begin = parent
		self.begin.title("DiffMeridionalMean.py, choose input data...")
		# Things that need defaults
		self.control_filepath = control_file
		self.control_filename = ''
		if self.control_filepath is not '':
		...
		self.Show_Checkbutton.pack(anchor = 'center')
		# Run button
		self.Run_Button = Button(self.frame, text = "RUN", command = self.runDiffMeridionalMean, height = 1)
		self.Run_Button.pack(anchor = "center")
	...
	def runDiffMeridionalMean(self):
		...
		# Build the string line by line to pass into os.system()
		pythonloc = '/home/server/student/homes/kniezgod/.conda/envs/condagoda/bin/python'
		codeloc = '/home/server/student/homes/kniezgod/python/bin/pygoda/DiffMeridionalMean.py'
		execute = pythonloc + " " + codeloc
		execute += " -c " + self.control_filepath
		execute += " -t " + self.test_filepath
		execute += " -box " + self.box
