...
def h0dates(start_date, end_date):
	import datetime
	...
	dates = []
	start_date = datetime.date(year = 1950 + start_year, month = start_month, day = 1)
	end_date = datetime.date(year = 1950 + end_year, month = end_month, day = 1)	
...
def h1dates(start_date, end_date):
	...
	dates = []
	start_date = datetime.date(year = 1950 + start_year, month = start_month, day = start_day)
	end_date = datetime.date(year = 1950 + end_year, month = end_month, day = end_day)	
	...
	while d <= end_date:
		# Find the year
		YEAR = str(d.year - 1950)
		# Append the correct number of zeros to the front of year
		for i in range(4-len(YEAR)):
			YEAR = '0' + YEAR	

		# Append the same for month
		MONTH = str(d.month)
		for i in range(2-len(MONTH)):
			MONTH = '0' + MONTH	

		# Append the same for day
		DAY = str(d.day)
		for i in range(2-len(DAY)):
			DAY = '0' + DAY
		
		# Handle leap years cause datetime is stupid
		if (MONTH == "02") & (DAY == "29"):
			d += delta
			continue	
...
def h1ts(v, box = None, wd = '.'):
	...
	'''
	from pygoda import camgoda, isNotOutlier, findClimoFile
	import numpy as np
	...
	dates = []
	varhold = []
	print "\n\n\nExtracting data for " + v + "...\n\n\n"
	start_date = datetime.date(year = 1950, month = 1, day = 1)
	end_date = datetime.date(year = 1950, month = 12, day = 31)
	d = start_date
	delta = datetime.timedelta(days = 1)
	while d <= end_date:
		f = findClimoFile("*cam.h1.dailyClim-" + d.strftime('%m-%d'), directory = wd)
		try: 
			full_path = os.path.abspath(f[0])
			fname = f[1]
			nc = camgoda(full_path)
			print fname
			dates.append(d.strftime('%m-%d'))
			if v == "PRECT_d18O":
				hold = nc.PRECT_d18O(box = box)
			elif v == "PRECT_dD":
				hold = nc.PRECT_dD(box = box)
			elif v == "PRECT_dxs":
				hold = nc.PRECT_dxs(box = box)
			elif "d18OV" in v:
				try:
					pressure = int(v[-3:]) * 100
					nc.d18OV(box = box)
					hold = nc.isobar(pressure)
				except ValueError:
					print "Something went wrong with the d18OV calculation."
					varhold.append(np.nan)
					d += delta
					continue
			elif "dDV" in v:
				try:
					...
					print "Something went wrong with the dDV calculation."
					varhold.append(np.nan)
					d += delta
					continue
			elif "dxsV" in v:
				try:
					...
					print "Something went wrong with the dxsV calculation."
					varhold.append(np.nan)
					d += delta
					continue
			else: 
				try: # for 3d variable with isobars
					pressure = int(v[-3:]) * 100
					variable = str(v[:-3])
					nc.variable(variable, box = box)
					hold = nc.isobar(pressure)
				except ValueError: # all others and specials# for all regular 2d data
					hold = nc.variable(v, box = box)
			
			# Keep track of the variable average for each date
			varhold.append(np.nanmean(hold))
			d += delta
		except (IndexError, IOError, AttributeError):
			d += delta

	try:
		good = isNotOutlier(np.array(varhold))
		rm = np.array([m for m, b in enumerate(good) if not b])
		varhold[rm] = np.nan
	except (ValueError, TypeError):
		None
	return varhold, nc.long_name, nc.units, dates
...
def h0ts(v, box, wd = '.'):
	...
	for m in months:
		f = findClimoFile("*" + m + "_climo.nc", directory = wd)
		try: 
			full_path = os.path.abspath(f[0])
			fname = f[1]
			nc = camgoda(full_path)
			...
			if v == "PRECT_d18O":
				...
				try:
					...
					nc.d18OV(box = box)
					hold = nc.isobar(pressure)
					nc.long_name = nc.long_name + " at " + str(pressure/100) + " mb"
				except ValueError:
					print "Something went wrong with the d18OV calculation."
					varhold.append(np.nan)
					# d += delta
	...
	# except (ValueError, TypeError):
	# 	None
	return varhold, nc.long_name, nc.units
...
d18OV and dDV : returns 2d numpy array data. 
	...
	def QFLX_d18O(self, box = None):
		...
		qflx_h2o = np.array(qmask.where(abs(qmask) > .000008))
		qflx_h218o = np.array(xr.DataArray(qflx_h218o).where(abs(qmask) > .000008))
	...
	def QFLX_dD(self, box = None):
		...
		qflx_h2o = self.variable("QFLX_H2O", box, setData = False, math = False)
		qmask = xr.DataArray(qflx_h2o)
		qflx_h2o = np.array(qmask.where(abs(qmask) > .000005))
		qflx_hdo = np.array(xr.DataArray(qflx_hdo).where(abs(qmask) > .000005))
		self.var = "QFLX_dD"
...
This class has been replaced by camgoda, but is being kept in the code for legacy scripts that use this class
...
d18OV and dDV : returns 2d numpy array data. 
	...
	def isobar(self, pressure, verb = False):
		if self.var == '':
			print "No variable read in yet.\nUse self.variable() to read a variable first."
			return
		if self.vartype == "2d":
