...
for N, which in enumerate(seasons):
	...
	class PRECT_obj:
		def __init__(self):
			self.control = cntrldata.variables['PRECT'][0,:,:].squeeze() * 1000 * 60 * 60 * 24 # Convert to mm/day
			self.test = testdata.variables['PRECT'][0,:,:].squeeze() * 1000 * 60 * 60 * 24 # Convert to mm/day
			self.units = "mm/day"
			self.long_name = "Total precipitation rate"
	class PRECT_H2O_obj:
		def __init__(self):
			self.control = cntrldata.variables['PRECT_H2O'][0,:,:].squeeze() * 1000 * 60 * 60 * 24 # Convert to mm/day
			self.test = testdata.variables['PRECT_H2O'][0,:,:].squeeze() * 1000 * 60 * 60 * 24 # Convert to mm/day
			self.units = "mm/day"
			self.long_name = cntrldata.variables['PRECT_H2O'].long_name
	class PRECT_H218O_obj:
		def __init__(self):
			self.control = cntrldata.variables['PRECT_H218O'][0,:,:].squeeze() * 1000 * 60 * 60 * 24 # Convert to mm/day
			self.test = testdata.variables['PRECT_H218O'][0,:,:].squeeze() * 1000 * 60 * 60 * 24 # Convert to mm/day
			self.units = "mm/day"
			self.long_name = cntrldata.variables['PRECT_H218O'].long_name
	class PRECT_d18O_obj:
		def __init__(self):
			prect_h218o = PRECT_H218O_obj()
			prect_h2o = PRECT_H2O_obj()
			self.control = (prect_h218o.control/prect_h2o.control - 1) * 1000
			self.test = (prect_h218o.test/prect_h2o.test - 1) * 1000
			self.units = "delta 18O (permil)"
			self.long_name = "delta 18O for PRECT"
	class PRECT_HDO_obj:
		def __init__(self):
			self.control = cntrldata.variables['PRECT_HDO'][0,:,:].squeeze() * 1000 * 60 * 60 * 24 # Convert to mm/day
			self.test = testdata.variables['PRECT_HDO'][0,:,:].squeeze() * 1000 * 60 * 60 * 24 # Convert to mm/day
			self.units = "mm/day"
			self.long_name = cntrldata.variables['PRECT_HDO'].long_name
	class PRECT_dD_obj:
		def __init__(self):
			prect_hdo = PRECT_HDO_obj()
			prect_h2o = PRECT_H2O_obj()
			self.control = (prect_hdo.control/prect_h2o.control - 1) * 1000
			self.test = (prect_hdo.test/prect_h2o.test - 1) * 1000
			self.units = "delta D (permil)"
			self.long_name = "delta D for PRECT"
	class PRECT_dxs_obj:
		def __init__(self):
			prect_dd = PRECT_dD_obj()
			prect_d18o = PRECT_d18O_obj()
			self.control = prect_dd.control - 8*prect_d18o.control
			self.test = prect_dd.test - 8*prect_d18o.test
			self.units = "d-excess (permil)"
			self.long_name = "d-excess (delta_D - 8*delta_18O) for PRECT"
	class PRECC_obj:
		def __init__(self):
			self.control = cntrldata.variables['PRECC'][0,:,:].squeeze() * 1000 * 60 * 60 * 24 # Convert to mm/day
			self.test = testdata.variables['PRECC'][0,:,:].squeeze() * 1000 * 60 * 60 * 24 # Convert to mm/day
			self.units = "mm/day"
			self.long_name = testdata.variables['PRECC'].long_name
	class PRECL_obj:
		def __init__(self):
			self.control = cntrldata.variables['PRECL'][0,:,:].squeeze() * 1000 * 60 * 60 * 24 # Convert to mm/day
			self.test = testdata.variables['PRECL'][0,:,:].squeeze() * 1000 * 60 * 60 * 24 # Convert to mm/day
			self.units = "mm/day"
			self.long_name = cntrldata.variables['PRECL'].long_name
	class PRECSC_obj:
		def __init__(self):
			self.control = cntrldata.variables['PRECSC'][0,:,:].squeeze() * 1000 * 60 * 60 * 24 # Convert to mm/day
			self.test = testdata.variables['PRECSC'][0,:,:].squeeze() * 1000 * 60 * 60 * 24 # Convert to mm/day
			self.units = "mm/day"
			self.long_name = cntrldata.variables['PRECSC'].long_name
	class PRECSL_obj:
		def __init__(self):
			self.control = cntrldata.variables['PRECSL'][0,:,:].squeeze() * 1000 * 60 * 60 * 24 # Convert to mm/day
			self.test = testdata.variables['PRECSL'][0,:,:].squeeze() * 1000 * 60 * 60 * 24 # Convert to mm/day
			self.units = "mm/day"
	...
	class QFLX_obj:
		def __init__(self):
			self.control = cntrldata.variables['QFLX'][0,:,:].squeeze() * 1000
			self.test = testdata.variables['QFLX'][0,:,:].squeeze() * 1000
			self.units = "g-H2O/m2/s"
			self.long_name = cntrldata.variables['QFLX'].long_name
	class QFLX_H218O_obj:
		def __init__(self):
			self.control = cntrldata.variables['QFLX_H218O'][0,:,:].squeeze()
			self.test = testdata.variables['QFLX_H218O'][0,:,:].squeeze()
			self.units = testdata.variables['QFLX_H218O'].units
			self.long_name = cntrldata.variables['QFLX_H218O'].long_name
	class QFLX_HDO_obj:
		def __init__(self):
			self.control = cntrldata.variables['QFLX_HDO'][0,:,:].squeeze()
			self.test = testdata.variables['QFLX_HDO'][0,:,:].squeeze()
			self.units = testdata.variables['QFLX_HDO'].units
			self.long_name = cntrldata.variables['QFLX_HDO'].long_name
	class CLDHGH_obj:
		def __init__(self):
			self.control = cntrldata.variables['CLDHGH'][0,:,:].squeeze()
			self.test = testdata.variables['CLDHGH'][0,:,:].squeeze()
			self.units = testdata.variables['CLDHGH'].units
			self.long_name = testdata.variables['CLDHGH'].long_name
	class PS_obj:
		def __init__(self):
			self.control = cntrldata.variables['PS'][0,:,:].squeeze()
			self.test = testdata.variables['PS'][0,:,:].squeeze()
			self.units = testdata.variables['PS'].units
			self.long_name = cntrldata.variables['PS'].long_name
	class PSL_obj:
		def __init__(self):
			self.control = cntrldata.variables['PSL'][0,:,:].squeeze()
			self.test = testdata.variables['PSL'][0,:,:].squeeze()
	...
	class H2OV_obj:
		def __init__(self):
			self.control = cntrldata.variables['H2OV'][0,:,:,:].squeeze()
			self.test = testdata.variables['H2OV'][0,:,:,:].squeeze()
			self.units = cntrldata.variables['H2OV'].units
			self.long_name = cntrldata.variables['H2OV'].long_name
	class HDOV_obj:
		def __init__(self):
			self.control = cntrldata.variables['HDOV'][0,:,:,:].squeeze()
			self.test = testdata.variables['HDOV'][0,:,:,:].squeeze()
			self.units = cntrldata.variables['HDOV'].units
			self.long_name = cntrldata.variables['HDOV'].long_name
	class H218OV_obj:
		def __init__(self):
			self.control = cntrldata.variables['H218OV'][0,:,:,:].squeeze()
			self.test = testdata.variables['H218OV'][0,:,:,:].squeeze()
			self.units = cntrldata.variables['H218OV'].units
			self.long_name = cntrldata.variables['H218OV'].long_name
	class dDV_obj:
		def __init__(self):
			hdov = HDOV_obj()
			h2ov = H2OV_obj()
	...
	class Q_obj:
		def __init__(self):
			self.control = cntrldata.variables['Q'][0,:,:,:].squeeze() * 100
			self.test = testdata.variables['Q'][0,:,:,:].squeeze() * 100
			self.units = "g-H2O/kg"
			self.long_name = cntrldata.variables['Q'].long_name
	class Q850_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "Q", 85000) * 100
			self.test = ExtractVarAtPressure(testdata, "Q", 85000) * 100
			self.units = Q_units
			self.long_name = Q_long_name + " at 850 mb"
	class Q500_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "Q", 50000) * 100
			self.test = ExtractVarAtPressure(testdata, "Q", 50000) * 100
			self.units = Q_units
			self.long_name = Q_long_name + " at 500 mb"
	class Q200_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "Q", 20000) * 100
			self.test = ExtractVarAtPressure(testdata, "Q", 20000) * 100
			self.units = Q_units
			self.long_name = Q_long_name + " at 200 mb"
	class VQ_H2O_obj:
		def __init__(self):
			self.control = cntrldata.variables['VQ_H2O'][0,:,:,:].squeeze()
			self.test = testdata.variables['VQ_H2O'][0,:,:,:].squeeze()
			self.units = testdata.variables['VQ_H2O'].units
			self.long_name = cntrldata.variables['VQ_H2O'].long_name
	class VQ_H218O_obj:
		def __init__(self):
			self.control = cntrldata.variables['VQ_H218O'][0,:,:,:].squeeze()
			self.test = testdata.variables['VQ_H218O'][0,:,:,:].squeeze()
			self.units = testdata.variables['VQ_H218O'].units
			self.long_name = cntrldata.variables['VQ_H218O'].long_name
	# class VQ_d18O_obj:
	# 	def __init__(self):
	# 		self.control = (VQ_H218O.control/VQ_H2O.control - 1) * 1000
	# 		self.test = (VQ_H218O.test/VQ_H2O.test - 1) * 1000
	# 		self.units = VQ_H218O_units
	# 		self.long_name = VQ_H218O_long_name
	class VQ_HDO_obj:
		def __init__(self):
			self.control = cntrldata.variables['VQ_HDO'][0,:,:,:].squeeze()
			self.test = testdata.variables['VQ_HDO'][0,:,:,:].squeeze()
			self.units = testdata.variables['VQ_HDO'].units
			self.long_name = cntrldata.variables['VQ_HDO'].long_name
	# class VQ_dD_obj:
	# 	def __init__(self):
	# 		self.control = (VQ_HDO.control/VQ_H2O.control - 1) * 1000
	# 		self.test = (VQ_HDO.test/VQ_H2O.test - 1) * 1000
	# 		self.units = VQ_HDO_units
	# 		self.long_name = VQ_HDO_long_name
	class UQ_H2O_obj:
		def __init__(self):
			self.control = cntrldata.variables['UQ_H2O'][0,:,:,:].squeeze()
			self.test = testdata.variables['UQ_H2O'][0,:,:,:].squeeze()
			self.units = testdata.variables['UQ_H2O'].units
			self.long_name = "Zonal flux for H2O" # Had to hard code in these names because the netcdf files have the wrong long_name ---> need to correct this in the cam_diagnostics.f90 file
	class UQ_H218O_obj:
		def __init__(self):
			self.control = cntrldata.variables['UQ_H218O'][0,:,:,:].squeeze()
			self.test = testdata.variables['UQ_H218O'][0,:,:,:].squeeze()
			self.units = testdata.variables['UQ_H218O'].units
			self.long_name = "Zonal flux for H218O"
	# class UQ_d18O_obj:
	# 	def __init__(self):
	# 		self.control = (UQ_H218O.control/UQ_H2O.control - 1) * 1000
	# 		self.test = (UQ_H218O.test/UQ_H2O.test - 1) * 1000
	# 		self.units = UQ_H218O_units
	# 		self.long_name = "Zonal flux for H218O"
	class UQ_HDO_obj:
		def __init__(self):
			self.control = cntrldata.variables['UQ_HDO'][0,:,:,:].squeeze()
			self.test = testdata.variables['UQ_HDO'][0,:,:,:].squeeze()
			self.units = testdata.variables['UQ_HDO'].units
			self.long_name = "Zonal flux for HDO"
	# class UQ_dD_obj:
	# 	def __init__(self):
	# 		self.control = (UQ_HDO.control/VQ_H2O.control - 1) * 1000
	# 		self.test = (UQ_HDO.test/UQ_H2O.test - 1) * 1000
	# 		self.units = UQ_HDO_units
	# 		self.long_name = "Zonal flux for HDO"
	class V_obj:
		def __init__(self):
			self.control = cntrldata.variables['V'][0,:,:,:].squeeze()
			self.test = testdata.variables['V'][0,:,:,:].squeeze()
			self.units = testdata.variables['V'].units
			self.long_name = cntrldata.variables['V'].long_name
	class V850_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "V", 85000)
			self.test = ExtractVarAtPressure(testdata, "V", 85000)
			self.units = V_units
			self.long_name = "Meridional wind speed at 850 mb"
	class V500_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "V", 50000)
			self.test = ExtractVarAtPressure(testdata, "V", 50000)
			self.units = V_units
			self.long_name = "Meridional wind speed at 500 mb"
	class V200_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "V", 20000)
			self.test = ExtractVarAtPressure(testdata, "V", 20000)
			self.units = V_units
			self.long_name = "Meridional wind speed at 200 mb"
	class VT_obj:
		def __init__(self):
			self.control = cntrldata.variables['VT'][0,:,:,:].squeeze()
			self.test = testdata.variables['VT'][0,:,:,:].squeeze()
			self.units = testdata.variables['VT'].units
			self.long_name = cntrldata.variables['VT'].long_name
	class U_obj:
		def __init__(self):
			self.control = cntrldata.variables['U'][0,:,:,:].squeeze()
			self.test = testdata.variables['U'][0,:,:,:].squeeze()
			self.units = testdata.variables['U'].units
			self.long_name = cntrldata.variables['U'].long_name
	class U850_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "U", 85000)
			self.test = ExtractVarAtPressure(testdata, "U", 85000)
			self.units = U_units
			self.long_name = "Zonal wind speed at 850 mb"
	class U500_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "U", 50000)
			self.test = ExtractVarAtPressure(testdata, "U", 50000)
			self.units = U_units
			self.long_name = "Zonal wind speed at 500 mb"
	class U200_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "U", 20000)
			self.test = ExtractVarAtPressure(testdata, "U", 20000)
			self.units = U_units
			self.long_name = "Zonal wind speed at 200 mb"
	class UT_obj:
		def __init__(self):
			self.control = cntrldata.variables['UT'][0,:,:,:].squeeze()
			self.test = testdata.variables['UT'][0,:,:,:].squeeze()
			self.units = testdata.variables['UT'].units
			self.long_name = cntrldata.variables['UT'].long_name
	class T_obj:
		def __init__(self):
			self.control = cntrldata.variables['T'][0,:,:,:].squeeze()
			self.test = testdata.variables['T'][0,:,:,:].squeeze()
			self.units = testdata.variables['T'].units
			self.long_name = cntrldata.variables['T'].long_name
	class T850_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "T", 85000)
			self.test = ExtractVarAtPressure(testdata, "T", 85000)
			self.units = T_units
			self.long_name = "Temperature at 850 mb"
	class T500_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "T", 50000)
			self.test = ExtractVarAtPressure(testdata, "T", 50000)
			self.units = T_units
			self.long_name = "Temperature at 500 mb"
	class T200_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "T", 20000)
			self.test = ExtractVarAtPressure(testdata, "T", 20000)
			self.units = T_units
	...
	class OMEGA_obj:
		def __init__(self):
			self.control = cntrldata.variables['OMEGA'][0,:,:,:].squeeze()
			self.test = testdata.variables['OMEGA'][0,:,:,:].squeeze()
			self.units = testdata.variables['OMEGA'].units
			self.long_name = cntrldata.variables['OMEGA'].long_name
	class OMEGA850_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "OMEGA", 85000)
			self.test = ExtractVarAtPressure(testdata, "OMEGA", 85000)
			self.units = OMEGA_units
			self.long_name = OMEGA_long_name + " at 850 mb"
	class OMEGA500_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "OMEGA", 50000)
			self.test = ExtractVarAtPressure(testdata, "OMEGA", 50000)
			self.units = OMEGA_units
			self.long_name = OMEGA_long_name + " at 500 mb"
	class OMEGA200_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "OMEGA", 20000)
			self.test = ExtractVarAtPressure(testdata, "OMEGA", 20000)
			self.units = OMEGA_units
			self.long_name = OMEGA_long_name + " at 200 mb"
	class Z3_obj:
		def __init__(self):
			self.control = cntrldata.variables['Z3'][0,:,:,:].squeeze() / 1000 # Convert to km
			self.test = testdata.variables['Z3'][0,:,:,:].squeeze() / 1000 
			self.units = "km"
			self.long_name = cntrldata.variables['Z3'].long_name
	class Z3850_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "Z3", 85000) / 1000 # Convert to km
			self.test = ExtractVarAtPressure(testdata, "Z3", 85000) / 1000 # Convert to km
			self.units = Z3_units
			self.long_name = Z3_long_name + " at 850 mb"
	class Z3500_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "Z3", 50000) / 1000 # Convert to km
			self.test = ExtractVarAtPressure(testdata, "Z3", 50000) / 1000 # Convert to km
			self.units = Z3_units
			self.long_name = Z3_long_name + " at 500 mb"
	class Z3200_obj:
		def __init__(self):
			self.control = ExtractVarAtPressure(cntrldata, "Z3", 20000) / 1000 # Convert to km
			self.test = ExtractVarAtPressure(testdata, "Z3", 20000) / 1000 # Convert to km
			self.units = Z3_units
			self.long_name = Z3_long_name + " at 200 mb"
	class P_obj:
		def __init__(self):
			self.from pygoda import PressureCalc
	...
	# 		self.units = E_units
	# 		self.long_name = E_long_name + " at 850 mb"
	# class E500_obj:
	# 	def __init__(self):
	...
	# 		self.units = E_units
	# 		self.long_name = E_long_name + " at 500 mb"
	# class E200_obj:
	# 	def __init__(self):
	...
	# 		self.units = E_units
	# 		self.long_name = E_long_name + " at 200 mb"
	# class ES_obj:
	# 	def __init__(self):
	...
	class TTEND_TOT_obj:
		def __init__(self):
			self.control = cntrldata.variables['TTEND_TOT'][0,:,:,:].squeeze() * 60 * 60 * 24 * 365 # Convert to K/yr
			self.test = testdata.variables['TTEND_TOT'][0,:,:,:].squeeze() * 60 * 60 * 24 * 365
			self.units = "K/year"
			self.long_name = cntrldata.variables['TTEND_TOT'].long_name
	class DCQ_obj:
		def __init__(self):
			self.control = cntrldata.variables['DCQ'][0,:,:,:].squeeze()
			self.test = testdata.variables['DCQ'][0,:,:,:].squeeze()
	...
	class CMFDT_obj:
		def __init__(self):
			self.control = cntrldata.variables['CMFDT'][0,:,:,:].squeeze()
			self.test = testdata.variables['CMFDT'][0,:,:,:].squeeze()
			self.units = testdata.variables['CMFDT'].units
			self.long_name = cntrldata.variables['CMFDT'].long_name
	class CMFDQR_obj:
		def __init__(self):
			self.control = cntrldata.variables['CMFDQR'][0,:,:,:].squeeze()
			self.test = testdata.variables['CMFDQR'][0,:,:,:].squeeze()
			self.units = testdata.variables['CMFDQR'].units
			self.long_name = cntrldata.variables['CMFDQR'].long_name
	class DTCOND_obj:
		def __init__(self):
			self.control = cntrldata.variables['DTCOND'][0,:,:,:].squeeze()
			self.test = testdata.variables['DTCOND'][0,:,:,:].squeeze()
	...
	# A.control = cntrldata.variables['hyam'][:]
	# A.test = testdata.variables['hyam'][:]
	# B_control = cntrldata.variables['hybm'][:]
	# B_test = testdata.variables['hybm'][:]
	
# class psi500calc_obj: 
# 	def __init__(self):
# 		self.pressures_above500mb = P - 50000 < 0
# 		self.pressures_below500mb = P - 50000 > 0		
# 		self.last_pressure_before500mb = P[pressures_above500mb][-1]
# 		self.first_pressure_after500mb = P[pressures_below500mb][0]
