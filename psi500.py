...
# Files = ["F.C5.2deg.wiso.defaultSSTICE_kn002/F.C5.2deg.wiso.defaultSSTICE_kn002_ANN_climo.nc"]
Files = ["F.C5.2deg.wiso.defaultSSTICE_kn002/F.C5.2deg.wiso.defaultSSTICE_kn002_DJF_climo.nc", "F.C5.2deg.wiso.obs6kSST_kn003/F.C5.2deg.wiso.obs6kSST_kn003_DJF_climo.nc"]
...
for N, fi in enumerate(Files):
	...
	f_name = os.path.splitext(os.path.split(fi)[1])[0]
	...
	for timenum, t in enumerate(time):
		...
		for n, l in enumerate(lats):
			psi.append(zonal_V[:,n] * zonal_pressure[:,n] * np.cos(np.radians(l)) / g)
		...
		hold = pd.DataFrame({'File' : [f_name], 'Time' : [month], 'ITCZcenter' : [ITCZcenter], 'HadleyCellNorthernEdge' : [HadleyCellNorthernEdge], 'HadleyCellSouthernEdge' : [HadleyCellSouthernEdge], 'NorthernCellWidth' : [WidthNorthernCell], 'SouthernCellWidth' : [WidthSouthernCell]})
